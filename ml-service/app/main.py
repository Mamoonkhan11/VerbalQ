...
# --- STARTUP EVENT: NLTK DATA DOWNLOAD ---
@app.on_event("startup")
async def startup_event():
    """
    Download necessary NLTK data when the application starts.
    Logs OLLAMA_URL for debugging connection issues.
    """
    # Log OLLAMA_URL for debugging
    ollama_url = os.environ.get('OLLAMA_URL', 'http://localhost:11434')
    print(f" ML Service starting with OLLAMA_URL: {ollama_url}")
...
frontend_url = "https://verbalqii.netlify.app"
backend_url = "https://verbalq-backend-314a.onrender.com"
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://localhost:8001",  # Local ML service
    ],
...
# Include routers (no prefix - routers have their own prefixes)
...
if __name__ == "__main__":
    import uvicorn
    # Dynamic port for Railway/Render/Cloud deployment
    port = int(os.environ.get("PORT", 8000))
    
    print(f" Starting ML Service on host=0.0.0.0 port={port}")
    print(f" Environment: {'Cloud (PORT=' + str(port) + ')' if os.environ.get('PORT') else 'Local (default port 8000)'}")
